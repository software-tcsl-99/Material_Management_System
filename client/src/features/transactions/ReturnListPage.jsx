...
const ReturnListPage = () => {
  ...
  const getConditionBadge = (condition) => {
    switch (condition?.toLowerCase()) {
      case 'good':
        return (
          <span className="inline-flex items-center gap-1 text-[10px] font-bold text-emerald-655 bg-emerald-50 dark:bg-emerald-950/20 px-2 py-0.5 rounded-full tracking-wider">
      ...
      case 'needs_repair':
        return (
          <span className="inline-flex items-center gap-1 text-[10px] font-bold text-amber-600 bg-amber-50 dark:bg-amber-950/20 px-2 py-0.5 rounded-full tracking-wider">
            <AlertTriangle className="w-3 h-3 text-amber-600" /> {condition?.replace('_', ' ')}
          </span>
        );
      default:
        return <span className="capitalize font-semibold text-slate-550 dark:text-slate-300">{condition}</span>;
    }
  };
      ...
      {loading ? (
        ...
        <div className="flex flex-col gap-6">
          {Object.entries(groupedReturns).map(([txnId, items]) => (
            ...
            >
              {/* Group Header */}
              <div className="px-5 py-4 bg-slate-50 dark:bg-slate-700 border-b border-slate-200 dark:border-slate-800 flex flex-wrap items-center justify-between gap-3">
                <div className="flex items-center gap-2">
                  <Layers className="w-4 h-4 text-blue-600" />
                  <span className="text-[10px] text-slate-400 dark:text-slate-200 font-extrabold tracking-wider block">Parent Transaction</span>
                  <span
                    onClick={() => navigate(`/transactions/${txnId}`)}
                    className="font-bold text-slate-800 dark:text-slate-100 hover:underline cursor-pointer font-mono text-sm tracking-wide"
                  >
                    {txnId}
                  </span>
                </div>
                <Badge variant="warning" className="text-[10px] font-bold px-2.5 py-0.5 tracking-wider">
                  ...
                  <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                    {items.map((row) => (
                      ...
                      >
                        ...
                        <td className="px-5 py-3.5 whitespace-nowrap dark:text-slate-100">
                          {getConditionBadge(row.condition)}
                        </td>
                        <td className="px-5 py-3.5 whitespace-nowrap">
                          {getStatusBadge(row.status)}
                        </td>
                        <td className="px-5 py-3.5 whitespace-nowrap text-slate-500 dark:text-slate-200 font-medium">
